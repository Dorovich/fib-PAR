...
void mandelbrot(int height, int width, double real_min, double imag_min,
    ...
    #pragma omp parallel
    #pragma omp single
	//#pragma omp taskloop /* BY ROWS WITH TASKLOOP */
    for (int row = 0; row < height; ++row) {
		//#pragma omp taskloop nogroup firstprivate(row) /* BY COLUMN WITH TASKLOOP (with/without nogroup) */
		#pragma omp task firstprivate(row) /* BY ROWS WITHOUT TASKLOOP */
        for (int col = 0; col < width; ++col) {
            //#pragma omp task firstprivate(row, col, display) /* BY COLUMN WITHOUT TASKLOOP */
			// {
            complex z, c;
